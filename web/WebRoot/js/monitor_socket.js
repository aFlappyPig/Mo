...
var builder4 = ProtoBuf.loadProtoFile("protobuf/test.proto");
...
var Hello = builder4.build("Test.Hello");
...
function responseAllUser(response){
	...
	for (var i = 0; i < response.userUpdate.length; i++) {
		...
		var image;
		if(response.userUpdate[i].userDetailInfo.userInfo.avatarId<10000) {
			image = "http://p6.sinaimg.cn/1698349933/180/31821309447041";
		}else {
		    var buffer = response.userUpdate[i].userDetailInfo.userInfo.avatar;
	...
	handleOnDataLoad(content);
}


function handleOnDataLoad(data) {
	...
	App.ListView = Ember.ListView
			.extend({
				...
				itemViewClass : Ember.ListItemView
						.extend({
							classNameBindings : [ 'isFirstObject:first-child' ],
							template : Ember.Handlebars
									.compile('<img {{bindAttr src="image"}} width="30" height="30"/>{{name}}{{#if view.isFirstObject}}(first child){{/if}}'),

							firstObjectBinding : Ember.Binding
									.oneWay('parentView.content.firstObject'),
							isFirstObject : function() {
								return Ember.guidFor(this.get('content')) === Ember
										.guidFor(this.get('firstObject'));
